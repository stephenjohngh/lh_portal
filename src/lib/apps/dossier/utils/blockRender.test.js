...
const { ASSET_FRAME_SANDBOX } = await import('./assetPreview.js');
...
describe('sanitizeBlockHtml — the iframe/img allow-rule', () => {
  ...
  it('re-asserts the sandbox even if the stored HTML dropped it', () => {
    ...
    const html = sanitizeBlockHtml('<iframe src="/api/media/file/abc"></iframe>');
    expect(html).toContain(`sandbox="${ASSET_FRAME_SANDBOX}"`);
  });

  it('pins the sandbox to our value, not whatever the stored markup asks for', () => {
    // The sandbox is not the security boundary (see ASSET_FRAME_SANDBOX), but
    // it must still be exactly one known value rather than author-controlled —
    // e.g. stored markup must not be able to add allow-top-navigation.
    const html = sanitizeBlockHtml(
      '<iframe src="/api/media/file/abc" sandbox="allow-top-navigation allow-popups"></iframe>');
    expect(html).toContain(`sandbox="${ASSET_FRAME_SANDBOX}"`);
