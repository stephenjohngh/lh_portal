...
describe('sanitizeBlockHtml — the iframe/img allow-rule', () => {
  ...
  it('never grants allow-same-origin, however the stored markup asks', () => {
    // allow-scripts + allow-same-origin together lets a frame remove its own
    // sandbox, which would put user-uploaded bytes inside this app's origin.
    const html = sanitizeBlockHtml(
      '<iframe src="/api/media/file/abc" sandbox="allow-scripts allow-same-origin"></iframe>');
    expect(html).toContain('sandbox="allow-scripts"');
