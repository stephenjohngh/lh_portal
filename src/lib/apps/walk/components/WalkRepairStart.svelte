...
<script>
  ...
  // FIX: filter on 'failed' || 'problem' — 'replace' was a V204 typo
  $: repairElements = buildingPlans.flatMap(plan =>
    ...
    const floorDiff = getFloorOrder(a._floor) - getFloorOrder(b._floor);
    ...
    <section class="grp">
      ...
      {:else if repairElements.length === 0}
        <div class="empty-repair">
          ...
          <div class="empty-txt">No failed or problem elements in {selectedBuilding}</div>
        </div>
      {:else}
        <p class="hint">{repairElements.length} element{repairElements.length !== 1 ? 's' : ''} need{repairElements.length == 1 ? 's' : ''} attention — tap to check repair </p>
        <div class="el-list">
          {#each repairElements as el (el.id)}
            <button class="el-btn" class:is-problem={el.status === 'problem'}
                    on:click={() => handleSelectElement(el)} disabled={saving}>
              ...
              <div class="el-body">
                ...
                <div class="el-meta">Floor {el._floor}{#if el.subtype} · {el.subtype}{/if}</div>
              </div>
