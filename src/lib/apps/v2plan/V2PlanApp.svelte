<script>
  // src/lib/apps/v2plan/V2PlanApp.svelte
  ...
  let state = {
    ...
    currentFloor: null, currentPlan: null, components: [], spaces: [], inspections: {},
  ...
  const unsub = v2planStore.subscribe(s => { state = s; });
  onDestroy(() => unsub());
  ...
  async function tryRefresh() {
    if (isOnline) await v2planStore.refresh();
  }

  // ── Cache age label (reactive) ────────────────────────────────────────────────

  $: cacheLabel = state.cachedAt
    ? v2planStore.ageLabel(Date.now() - state.cachedAt.getTime())
    : null;
  ...
  onMount(async () => {
    if ($auth.user) {
      await permissions.init($auth.user.id, 'v2plan');
    }

    window.addEventListener('online',  handleOnline);
    window.addEventListener('offline', handleOffline);

    await v2planStore.load();
  });
...
<style>
  /* Import DM Mono font (matches v2walk) */
