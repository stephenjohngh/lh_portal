...
<script>
  import { allSlaClocks, BSR_TRACK_STATUSES } from '$lib/apps/mor/utils/morHelpers';
  import { fmtDateTime } from '$lib/utils/dates';

  export let c; // mor_cases row

  $: clocks = allSlaClocks(c);
    ...
    {#each clocks.filter(cl => cl.id !== 'bsr_report') as item}
      <div class="rounded border p-2 text-center
                  {item.done ? 'border-slate-700 bg-slate-800/30' :
                   item.clock ? clockBg(item.clock.status) : 'border-slate-700 bg-slate-800/30'}">
        <p class="text-xs text-slate-500 mb-0.5">{item.label}</p>
        {#if item.done}
          <p class="text-xs text-emerald-400">✓ Done</p>
        {:else if item.clock}
          <p class="text-xs font-semibold {clockText(item.clock.status)}">{item.clock.label}</p>
        {:else}
