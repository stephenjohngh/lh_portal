<!-- src/lib/apps/v2walk/components/V2WalkComponentEditor.svelte -->
...
<script>
  ...
  $: {
    const compAttrs = $v2walkStore.allComponentAttrs?.[component?.id] ?? [];
    attrValues = {};
  ...
  async function handleSave() {
    ...
    try {
      await v2walkStore.updateComponent(component.id, {
