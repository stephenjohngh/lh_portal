<!-- src/lib/apps/v2walk/components/V2WalkHome.svelte -->
<script>
  ...
  function typeColor(s) {
    return { test: 'orange', inspection: 'blue', repair: 'red' }[s.session_type] ?? 'grey';
  }
    ...
    <div class="sect">
      ...
      <button class="start-btn start-repair" on:click={() => dispatch('startRepair')}>
        ...
        <div class="start-text">
          ...
          <div class="start-sub">Inspect Repairs to Failed Components</div>
        </div>
      </button>
    </div>
  {/if}
...
<style>
  .home { display:flex; flex-direction:column; min-height:100vh; padding-bottom:2rem; background:#0d0d14; color:#f0f0f0; font-family:'DM Mono','Courier New',monospace; }
  ...
  .pulse { width:9px; height:9px; border-radius:50%; background:#22c55e; flex-shrink:0; animation:pulse-anim 1.5s ease-in-out infinite; }
  ...
  .session-list-scroll { max-height:10.5rem; overflow-y:auto; -webkit-overflow-scrolling:touch; }
  .scard { width:100%; display:flex; align-items:center; gap:0.875rem; padding:0.875rem 1rem; text-align:left; background:#111122; border:2px solid #2e2e42; border-radius:10px; cursor:pointer; font-family:inherit; transition:border-color 0.15s; }
  ...
  .resume-label { font-size:0.68rem; font-weight:800; letter-spacing:0.1em; color:#4ade80; flex-shrink:0; white-space:nowrap; }
