<!-- src/lib/apps/v2walk/components/V2WalkSessionStart.svelte -->
...
<script>
  ...
  async function handleBegin() {
    ...
    try {
      ...
      if (scope === 'single_floor') {
        await v2walkStore.startSession({ ...opts, floor: selectedFloor });
      } else {
        await v2walkStore.startBuildingWideSession(opts);
      }
      dispatch('started');
    } catch (err) {
