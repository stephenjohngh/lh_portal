...
<Modal show={true} size="medium" on:close={handleClose}>
  ...
  <div class="section-spacing">
    <div>
      ...
      <input id="plan-name" type="text" bind:value={formData.name}
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <input id="building" type="text" bind:value={formData.building}
          placeholder="e.g., Building A"
          class="input" class:error={errors.building} />
        {#if errors.building}<p class="field-error">{errors.building}</p>{/if}
      </div>
      <div>
        <label for="floor-level" class="block text-sm font-medium mb-2">Floor Level</label>
        <select id="floor-level" bind:value={formData.floor_level} class="select w-full">
  ...
  <div slot="footer" class="flex items-center justify-between">
    <Button variant="danger" size="medium" icon="delete" on:click={() => showDeleteConfirm = true}>
      Delete Plan
    </Button>
    <div class="btn-group">
      <Button variant="secondary" size="medium" on:click={handleClose} disabled={saving}>Cancel</Button>
...
<ConfirmDialog
  ...
  message="Are you sure you want to delete this floor plan? This will also delete all {elementCount} elements. This action cannot be undone."
