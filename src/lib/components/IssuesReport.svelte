...
<script>
  ...
  // Get all current issues with their outstanding actions
  $: currentIssues = issues
    .filter(issue => issue.status === 'current' || !issue.status) // Only current issues
    .map(issue => {
  ...
  <div class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
    <div class="bg-white w-full max-w-5xl h-[90vh] flex flex-col rounded-lg shadow-2xl">
      ...
      <div class="flex justify-between items-center p-6 border-b border-gray-300 print:hidden bg-slate-800 text-white rounded-t-lg">
        <h2 class="text-2xl font-bold">Issues Report</h2>
      ...
      <div class="flex-1 overflow-y-auto p-8 bg-white text-gray-900">
        ...
        {#if currentIssues.length === 0}
          <div class="text-center py-12">
            ...
            <p class="text-xl text-gray-600">No current issues found!</p>
          </div>
        {:else}
          <!-- Summary Stats -->
          <div class="grid grid-cols-3 gap-4 mb-8 print:mb-6">
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
              ...
              <div class="text-sm text-blue-600">Current Issues</div>
            </div>
            <div class="bg-orange-50 p-4 rounded-lg border border-orange-200">
              <div class="text-2xl font-bold text-orange-700">
                {currentIssues.reduce((sum, issue) => sum + issue.outstandingActions.length, 0)}
              </div>
              <div class="text-sm text-orange-600">Outstanding Actions</div>
            </div>
            <div class="bg-red-50 p-4 rounded-lg border border-red-200">
              <div class="text-2xl font-bold text-red-700">
                {currentIssues.reduce((sum, issue) => 
          ...
          <!-- Issues List -->
          <div class="space-y-6">
            {#each currentIssues as issue, index}
